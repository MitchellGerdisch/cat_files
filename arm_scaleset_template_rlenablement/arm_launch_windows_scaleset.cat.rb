...
name 'Launch Windows ARM Scale Set'
rs_ca_ver 20160622
short_description "![logo](https://s3.amazonaws.com/rs-pft/cat-logos/azure.png)

Launch Windows ARM scale set"
long_description "Uses an ARM template to launch an App scale set of Windows VMs."


import "arm/api_common"
import "arm/api_template"
import "arm/windows/template", as: "template"
...
parameter "param_os_version" do
  ...
  type "string" 
  description "Version of OS to use for scale set VMs." 
  default "2016-Datacenter"
  allowed_values "2016-Datacenter"
end
...
define arm_deployment_launch($param_environment, $param_instance_type, $param_os_version, $param_scaleset_name, $param_instance_count, $param_server_username, $param_server_password) do
  ...
  $servertemplate_href = "/api/server_templates/391125003"  # RightLink 10.5.0 Windows Base 
  ...
  call template.build_arm_template_launch_body($refresh_token, $uca_name, $servertemplate_href, $deployment_href, $param_instance_type, $param_os_version, $param_scaleset_name, $param_instance_count, $param_server_username, $param_server_password) retrieve $arm_template_launch_body
...
define cleanup_servers($target_instance_count) do
  ...
  while $num_keepers_found < to_n($target_instance_count) do
    ...
    foreach @server in @all_servers do
      if @server.state == "operational" || @server.state == "stranded" || @server.state == "stranded in booting"
  ...
  @stranded_servers = select(@servers_to_remove, {state: "stranded"}) + select(@servers_to_remove, {state: "stranded in booting"})
